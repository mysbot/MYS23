...
void WindowControl::controlAutoLiftTransmitter(Command command, uint16_t bufferIndex) {
  switch (command) {
    ...
    case Command::WINDOW_STOP:
    case Command::WINDOW_DOWN:
      rfTransmitter.sendCode(command, RF_buffer[bufferIndex], bufferIndex);  // 发送自动升降窗的命令
      break;
    ...
    case Command::RAIN_SIGNAL:
      if(ADDmanager.rainSignal_value)
      rfTransmitter.sendCode(Command::WINDOW_UP, RF_buffer[static_cast<uint8_t>(ControlGroup::GROUP2)-1],static_cast<uint8_t>(ControlGroup::GROUP2)-1);
      break;
    default:
      // Handle unknown commands
      break;
...
void WindowControl::controlCasementWindowTransmitter(Command command, uint16_t bufferIndex) {
  switch (command) {
    ...
    case Command::CASEMENT_CLOSE:   
    case Command::CASEMENT_CLOSE_ALT:
      rfTransmitter.sendCode(command, RF_buffer[bufferIndex], bufferIndex);
      break;
    case Command::RAIN_SIGNAL:
      if(ADDmanager.rainSignal_value){
        rfTransmitter.sendCode(Command::CASEMENT_CLOSE, RF_buffer[static_cast<uint8_t>(ControlGroup::GROUP1)-1],static_cast<uint8_t>(ControlGroup::GROUP1)-1);
        rfTransmitter.sendCode(Command::CASEMENT_CLOSE_ALT, RF_buffer[static_cast<uint8_t>(ControlGroup::GROUP2)-1],static_cast<uint8_t>(ControlGroup::GROUP2)-1);
      };
      break;
    default:
      // Handle unknown commands
